...
def graph(basename, subname, seconds):
    rrds=glob.glob(basename+"/io*.rrd")
	...
	cmdline+="GPRINT:id%dpercent:AVERAGE:\"%s\" "%(id,"Average IO usage\: %02.0lf\\j")

    # print cmdline
    os.system("nice -n+20 rrdtool graph %sxen-io-%s.png --start -%d --end -69 %s > /dev/null"%(folder,subname,seconds,cmdline))
